...
// 生成组合后的prompt
function generatePrompt(text, illustration, color) {
...
// 添加图像生成帮助文档资源
server.resource("help", "help://generate-image", async (uri) => ({
    contents: [{
            uri: uri.href,
...
// 添加视频生成帮助文档资源
server.resource("help", "help://generate-video", async (uri) => ({
    contents: [{
            uri: uri.href,
...
// 添加提交视频任务帮助文档资源
server.resource("help", "help://submit-video-task", async (uri) => ({
    contents: [{
            uri: uri.href,
...
// 添加查询视频任务帮助文档资源
server.resource("help", "help://get-video-task", async (uri) => ({
    contents: [{
            uri: uri.href,
...
// 注册图片生成工具
server.tool("generate-image", "当用户需要生成图片时使用的工具", {
    text: zod_1.z.string().describe("用户需要在图片上显示的文字"),
    illustration: zod_1.z.string().describe("根据用户要显示的文字，提取3-5个可以作为图片配饰的插画元素关键词"),
    color: zod_1.z.string().describe("图片的背景主色调"),
    ratio: zod_1.z.enum(["4:3", "3:4", "16:9", "9:16"]).describe("图片比例。支持: 4:3 (512*384), 3:4 (384*512), 16:9 (512*288), 9:16 (288*512)")
}, async (args, _extra) => {
    var _a, _b;
    ...
    try {
        ...
        // 获取LLM优化后的提示词
        const llmPrompt = ((_b = (_a = result.raw_response) === null || _a === void 0 ? void 0 : _a.data) === null || _b === void 0 ? void 0 : _b.rephraser_result) || prompt;
...
// 注册视频生成工具
server.tool("generate-video", "当用户需要生成视频时使用的工具，基于即梦AI文生视频模型。此工具会等待视频生成完成后返回结果（同步方式，内部轮询），预计耗时1-3分钟，最长等待约8分钟。", {
    prompt: zod_1.z.string().describe("视频内容的描述")
}, async (args, _extra) => {
    var _a, _b;
    const { prompt } = args;
    // 检查API密钥是否配置
    ...
    try {
        ...
        // 同步生成视频（内部会自动提交任务并轮询结果）
        log("视频生成中，请稍候（预计1-3分钟，最长等待约8分钟）...");
        ...
        const responseData = {
            ...
            data: {
                ...
                video_url: ((_a = result.video_urls) === null || _a === void 0 ? void 0 : _a[0]) || "",
                video_count: ((_b = result.video_urls) === null || _b === void 0 ? void 0 : _b.length) || 0,
        ...
        const errorData = {
            ...
            error: errorMsg,
            note: isTimeoutError
                ? "视频生成超时。视频生成通常需要1-3分钟，最长等待约8分钟，请检查网络连接或稍后重试"
                : "视频生成失败，请检查提示词和网络状态",
            timestamp: new Date().toISOString()
        };
        return {
...
// ==================== 即梦AI新版模型工具 ====================
// 注册即梦4.0图片生成工具
server.tool("jimeng-v40-generate", "即梦4.0图片生成工具，支持文生图、图像编辑及多图组合生成（最多10张输入图，最多15张输出图），支持4K超高清输出。此工具会等待图片生成完成后返回结果。", {
